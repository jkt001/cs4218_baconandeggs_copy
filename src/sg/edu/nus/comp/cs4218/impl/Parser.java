...
public class Parser {
          ...
          if (thisChar == ';') {
            ...
            if (currentWord.length() > 0) {
              if (isFirstArg) {
                ...
                addToArgs(currentWord.toString());
              }
              currentWord = new StringBuilder();  
            }
          } else if (thisChar == '|') {
  ...
  private void addToArgs(String toAdd) {
    CharSequence cs = "*";
    if (toAdd.contains(cs)) {
      
      String[] partsOfPath = toAdd.split("*");
      //Cant tell if [0] is before or after *

      //Gets all files and add
      //Must handle different kind of globbing here
    } else {
      currentArgs.add(toAdd);
    }
  }
