...
public class CdApplicationTest {
	...
	/*
	 * Test various types of paths that can be specified by the user, e.g.
	...
	public void testAbsolutePathWithOneFolderWithExtraSlashThatExists() {
		if (isWindows()) {
			testCdExpectSuccess(USER_DIR, WIN_WINDOWS_DIR + "\\",
					WIN_WINDOWS_DIR);
		} else if (isMac()) {
			testCdExpectSuccess(USER_DIR, MAC_SYSTEM_DIR + "/", MAC_SYSTEM_DIR);
		} else {
	...
	public void testRelativePathUpTwoLevels() {
		if (isWindows()) {
			testCdExpectSuccess(WIN_WINDOWS_DIR + "\\System32",
					WIN_GRANDPARENT, WIN_DRIVE_ROOT);
		} else if (isMac()) {
			testCdExpectSuccess(MAC_SYSTEM_DIR + "/Library", UNIX_GRANDPARENT,
					UNIX_ROOT);
		} else {
			testCdExpectSuccess(UNIX_VAR_DIR + "/tmp", UNIX_GRANDPARENT,
					UNIX_ROOT);
		}
	}
	...
	public void testRelativePathToParentAndSubdirectory() {
		if (isWindows()) {
			testCdExpectSuccess(WIN_WINDOWS_DIR + "\\System32", "..\\System",
					WIN_WINDOWS_DIR + "\\System");
		} else if (isMac()) {
			testCdExpectSuccess(MAC_SYSTEM_DIR + "/Library/Java",
					"../Spotlight", MAC_SYSTEM_DIR + "/Library/Spotlight");
		} else {
	...
	public void testRootRelativePathWithSubdirectory() {
		if (isWindows()) {
			...
			testCdExpectSuccess(MAC_SYSTEM_DIR, "/Applications",
					"/Applications");
		} else {
	...
	public void testRelativePathBeyondRootByGoingUpTwoLevels() {
		if (isWindows()) {
			testCdExpectSuccess(WIN_WINDOWS_DIR, WIN_GRANDPARENT,
					WIN_DRIVE_ROOT);
		} else if (isMac()) {
	...
	public void testDirectoryWithNoPermissionsToListContentsOf()
		...
		Path tempFolderPath = FileSystems.getDefault().getPath(USER_DIR,
				tempFolderName);
	...
	private void testCdExpectSuccess(String initialDirectory, String cdPath,
